...
function Top5DocsRechAp() {
  ...
  const renderHeader = () => {
    let headerElement = ['APLICANTE', 'DOCUMENTOS RECHAZADOS']
  ...
  const renderBody = () => {
    return departamentos.map(({ APLICANTE, RECHAZOS }) => {
      return (
        <tr key={APLICANTE}>
          <td>{APLICANTE}</td>
          <td>{RECHAZOS}</td>
  ...
  return (
    <>
      ...
      <br />
      <h1 style={{ textAlign: "center", color: "white" }}>Top 5 Aplicantes con mas Documentos Rechazados</h1>
      <br />
