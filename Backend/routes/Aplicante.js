...
router.post("/insertAplicante", async function (req, res, next) {
  ...
  //se genera una contraseña la que será el tiempo actual
  const contrasenia = Date.now();
  let now = new Date();
  ...
  let respInsApl = await service.connect(
    `INSERT INTO APLICANTE_EMPLEADO VALUES
    (${dpi},'${nombres}','${apellidos}','${contrasenia}','${correo}','${direccion}','${telefono}','pendiente','${fechainicio}','-','${revisor}','${depart}','${puesto}')`
  );
...
router.post("/getRevisor", async function (req, res, next) {
  ...
  // SELECT * FROM COORDINADOR_REVISOR t WHERE t.ParaRevisar = ( SELECT MIN( ParaRevisar )  FROM COORDINADOR_REVISOR) AND t.Tipo='Revisor' AND t.DEPARTAMENTO = 'uno'
  let respGetRev = await service.connect(
    `SELECT Nombre FROM COORDINADOR_REVISOR t WHERE t.ParaRevisar = ( SELECT MIN( ParaRevisar )  FROM COORDINADOR_REVISOR) AND t.Tipo='Revisor' AND t.DEPARTAMENTO = '${departamento}'`
  );
